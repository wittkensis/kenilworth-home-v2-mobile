      ...
      <form action={handleSave} className="sheet-form">
        <div className="sheet-body">
          <div className="form-fields">
            <Field label="Name">
              <input name="name" defaultValue={asset?.name ?? ''} required placeholder="e.g. Furnace" className="input" />
            </Field>

            <div className="form-fields-row">
              <Field label="Brand">
                <input name="brand" defaultValue={asset?.brand ?? ''} placeholder="e.g. Lennox" className="input" />
              </Field>
              <Field label="Model">
                <input name="model" defaultValue={asset?.model ?? ''} placeholder="Optional" className="input" />
              </Field>
            </div>
            ...
            {selectedGroupId && filteredItems.length > 0 && (
              <Field label="Sub-area">
                <select name="area_item_id" defaultValue={asset?.area_item_id?.toString() ?? ''} className="input select">
            ...
            <div className="form-fields-row">
              <Field label="Purchase Date">
                <input name="purchase_date" type="date" defaultValue={asset?.purchase_date ?? ''} className="input" />
              </Field>
              <Field label="Warranty Expires">
                <input name="warranty_expires" type="date" defaultValue={asset?.warranty_expires ?? ''} className="input" />
              </Field>
            </div>

            <Field label="Manual URL">
              <input name="manual_url" type="url" defaultValue={asset?.manual_url ?? ''} placeholder="https://..." className="input" />
            </Field>

            <Field label="Notes">
              <textarea name="notes" defaultValue={asset?.notes ?? ''} placeholder="Optional notes" className="input textarea" />
            </Field>
          </div>
        </div>
